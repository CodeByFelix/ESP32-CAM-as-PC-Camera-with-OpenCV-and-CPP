    ...
    while (true)
    {
        ...
        if (w == 's')
        {
            ...
            if(!recording)
            {
                ...
                std::cout << "\n\nFrame Rate = " << cam.get(cv::CAP_PROP_FPS) << std::endl;
                videoWriter.open(fileName, cv::VideoWriter::fourcc('M', 'J', 'P', 'G'), 8, cv::Size(frameWidth, frameHeight));
        ...
        if (recording)
        {
            ...
            cv::putText(frame, "Recording", cv::Point(40, 60), cv::FONT_HERSHEY_SIMPLEX, 1, cv::Scalar(0, 0, 255), 1);
        }
